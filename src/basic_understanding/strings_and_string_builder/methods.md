...
> `\d` represents any digit in regular expression but `\` is a backlash character in java so to convert `\` into a
> literal we add another `\` so that `\d` can be considered as literal in regular expression!
